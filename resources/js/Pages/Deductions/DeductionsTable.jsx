...
const EditableCell = ({ 
    ...
    if (!isEditing) {
        return (
            <div 
                className={`p-2 text-right ${deductionExists ? 'text-gray-700' : 'text-gray-400'} cursor-pointer hover:bg-gray-100`}
                onClick={(e) => {
                    e.stopPropagation();
                    if (deductionExists) {
                        if (onClick) onClick();
                ...
                style={{ minWidth: '80px', overflow: 'visible' }}
            >
                {deductionExists ? parseFloat(value).toFixed(2) : "0.00"}
            </div>
        );
    }
                                    ...
                                    {fieldColumns.map((column, colIndex) => (
                                        ...
                                        >
                                            <EditableCell 
                                                ...
                                                colIndex={colIndex}
                                                field={column.id}
                                                onClick={() => !isPosted && deduction && handleEditCell(employee.id, deduction.id, column.id, virtualRow.index, colIndex)}
